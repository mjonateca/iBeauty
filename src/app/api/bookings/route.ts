...
export async function POST(request: Request) {
  ...
  if (isManual && !["shop_owner", "barber"].includes(account.role)) {
    return NextResponse.json({ error: "Solo el salón de belleza o un profesional pueden crear reservas manuales" }, { status: 403 });
  }
  ...
  if (!isSubscriptionAccessible(subscription?.status, subscription?.current_period_end)) {
    return NextResponse.json(
      { error: "El salón de belleza tiene la suscripción vencida y no puede recibir nuevas reservas." },
      { status: 409 }
    );
  }

  if (!barber?.is_active || barber.shop_id !== parsed.data.shop_id) {
    return NextResponse.json({ error: "Estilista no disponible en este salón de belleza" }, { status: 409 });
  }

  if (!service?.is_active || service.is_visible === false || service.shop_id !== parsed.data.shop_id) {
    return NextResponse.json({ error: "Servicio no disponible en este salón de belleza" }, { status: 409 });
  }

  if (isManual) {
    const ownsShop = account.role === "shop_owner" && shop.owner_id === user.id;
    const ownsBeautyAgenda = account.role === "barber" && barber?.user_id === user.id;
    if (!ownsShop && !ownsBeautyAgenda) {
  ...
  if (daySchedule?.closed) {
    return NextResponse.json({ error: "El salón de belleza está cerrada ese día" }, { status: 409 });
  }
  ...
  if (
    ...
    return NextResponse.json({ error: "La hora elegida está fuera del horario del salón de belleza" }, { status: 409 });
  }
