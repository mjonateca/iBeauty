...
export async function GET(request: Request) {
  ...
  for (const event of events) {
    ...
    if (!normalizeReminderChannels(booking.shops?.reminder_channels).includes("email")) {
      await admin.from("notification_events").update({ status: "skipped", error: "El correo está desactivado para este salón de belleza" }).eq("id", event.id);
      skipped += 1;
      continue;
    }
    ...
    const html = buildReminderEmailHtml({
      clientName,
      shopName: shop?.name || "Tu salón de belleza",
      barberName,
      serviceName,
      date: booking.date,
    ...
    const result = await resend.emails.send({
      from: `${shop?.name || "iBeauty"} <${fromAddress}>`,
      to: recipientEmail,
